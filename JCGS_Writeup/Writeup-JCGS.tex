...
throughout the data cleaning process and their impact on the tile plots}
...
bullets was recreated using our interactive framework, shown in the top
tile plot of Figure~\ref{fig-matrixDAdjustments}.
...
deviation in performance according to the matrix shown in the bottom
plot of Figure~\ref{fig-matrixDAdjustments}. While there may appear to
be a dip in performance around this area, the differentiation happens in
...
\(B_2\) and \(B_1\) at a given \(d\). \(\theta_0\) gives a baseline
score for each individual barrel, while \(\theta_1\) represent the
coefficient associated with the change in mean \(CCF_{max}\) score of
bullets at a given \(d\).

It is apparent that there is differences in baseline model performance
between different firearms. \(\theta_0\) ranges from 0.64291 for barrel
B to 0.73287 for barrel J, with a mean of 0.67228.

We hypothesized that model performance would decline as the distance
between shots fired increases, leading \(\theta_1\) to be negative. This
is true in most firearms, but \(\theta_1\) is positive for firearms 2,
D, and J. Overall, however, \(\theta_1\) is relatively small among all
barrels, falling between -0.00268 and 0.00084 , with a mean of -0.00083.
Combined with the visual analysis of the variogram, we conclude that
bullet distance has minimal effect on model accuracy relative to other
causes of variability. This is evident when \(d < 30\), whereas beyond
\(d > 30\), there is insufficient data to make definitive claims.

Furthermore, we acknowledge that there is significant variability among
the in-phase scores across the firearms we examined. This variability
suggests that additional factors beyond bullet distance have a
substantial effect on model performance and accuracy

While the distance between shots may not be a significant factor in
predictive performance, we recognize there is significant amount of
variability among in-phase scores between shots. Potential factors may
include, but are not limited to, inconsistencies in bullet scan
...
Future research in this area should focus on exploring the underlying
causes of variability between shots and identifying the factors
contributing to this variability. This is crucial to improving model
performance and establishing confidence within the forensic community
regarding the adoption of such algorithmic models. Additionally,
exploring the impact of firearm storage on bullet performance presents a
