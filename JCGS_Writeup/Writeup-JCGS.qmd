...
It is apparent that there is differences in baseline model performance between different firearms. $\theta_0$ ranges from `r min(model_table$intercept)` for barrel B to `r format(max(model_table$intercept), scientific = FALSE)` for barrel J, with a mean of `r sprintf("%.5f", mean(model_table$intercept))`.

We hypothesized that model performance would decline as the distance between shots fired increases, leading $\theta_1$ to be negative. This is true in most firearms, but $\theta_1$ is positive for firearms 2, D, and J. Overall, however, $\theta_1$ is relatively small among all barrels, falling between `r min(model_table$slope)` and `r sprintf("%.5f", max(model_table$slope))`
