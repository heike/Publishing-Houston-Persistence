...
XX firearm and barrel - need a note specifying that we use this here exchangeably
...
Current forensic practices rely on examiners to visually inspect bullets under a comparison microscope for similarities of marks on the bullets' surfaces. As a bullet is discharged from the firearm, the rifling in the barrel forces the bullet to follow the grooves - like rails. Micro-imperfections in the barrel leave scratches (called *striations*) on the bullet's surface. Striations on land engraved areas (LEA; the area between two grooves) are assumed to be unique to the individual firearm. This allows forensic examiners to determine whether two bullets originate from the same source by seeing if these LEAs match [@afte]. However, this process is highly subjective, and relies heavily on an examiner's expertise, leading to criticism by the @nas2009 and the @pcast. In response, algorithms for assessing  the similarity of firearms evidence were developed [@carriquiryMachineLearningForensic2019; @chenFiredBulletSignature2019; @chuAutomaticIdentificationBullet2013; @juOpenSourceImplementationCMPS2022; @vorburgerApplicationsCrosscorrelationFunctions2011; @vorburgerTopographyMeasurementsApplications2015] to provide objective measures with the goal of augmenting an examiner's testimony.
...
It is apparent that there is differences in baseline model performance between different firearms. $\theta_0$ ranges from `r round(min(model_table$intercept),2)` for barrel B to `r round(max(model_table$intercept), 2)` for barrel J, with a mean of `r sprintf("%.2f", mean(model_table$intercept))`.

We expect that the similarity between shots will decline as the distance between shots  increases, leading $\theta_1$ to be negative. We see this performance in most firearms, Overall, however, $\theta_1$ is relatively small among all barrels, falling between `r min(model_table$slope)` and `r sprintf("%.5f", max(model_table$slope))`. The slight rise in firearms D and J is due to 
<--This is true in most firearms, but $\theta_1$ is positive for firearms 2, D, and J.--> 
, with a mean of `r sprintf("%.5f", mean(model_table$slope))`. Combined with the visual analysis of the variogram, we conclude that bullet distance has minimal effect on model accuracy relative to other causes of variability. This is evident when $d < 30$, whereas beyond $d > 30$, there is insufficient data to make definitive claims.

Furthermore, we acknowledge that there is significant variability among the in-phase scores across the firearms we examined. This variability suggests that additional factors beyond bullet distance have a substantial effect on model performance and accuracy
