...
keys <- rbbt::bbt_detect_citations("Writeup-JCGS.qmd")
#keys <- grep("fig-", keys, invert=TRUE, value = TRUE)
#keys <- grep("sec-", keys, invert=TRUE, value = TRUE)

get_bibs_from_zotero <- function(keys = keys, library_id, 
                                 ignore = c("fig-", "sec-"), translator="bibtex") {
  force(keys)
  # Is Zotero running?
  rbbt::assert_bbt()
  # remove anything we should ignore
  for (ig in ignore) {
    keys <- grep(ig, keys, invert=TRUE, value = TRUE)
  }
  if (length(keys) == 0) return("")
  if (length(library_id) == 0) return("")
  
  if (is.character(library_id)) {
    library_id = sapply(library_id, FUN=rbbt::bbt_library_id)
  }
  # now load citations from each id
  bibs <- keys |> sapply(FUN = function(key) {
    res <- library_id |> sapply(FUN = function(libID) {
      try(rbbt::bbt_bib(key, translator, 
        library_id = libID, .action = identity), silent = TRUE)
      })
    grep("Error : not found:", res, value = TRUE, invert = TRUE)
  })
  not_found <- setdiff(keys, names(bibs))
  if (length(not_found) > 0) 
    message(sprintf("Citations not found: %s", paste(not_found, collapse = ", ")))
  bibs
}

bibs <- get_bibs_from_zotero(keys, library_id = c("CSAFE", "Graphics-Research"))
readr::write_file(paste(bibs, collapse="\n"), "bibliography.bib")
```
...
# HH: 1 is not always providing good random numbers - some methods need a bit of startup for small seeds, use something that's five or six digits long.
set.seed(654321)
...
Current forensic practices rely on examiners to visually inspect bullets under a comparison microscope for similarities of marks on the bullets' surfaces. As a bullet is discharged from the firearm, the rifling in the barrel forces the bullet to follow the grooves - like rails. Micro-imperfections in the barrel leave scratches (called *striations*) on the bullet's surface. Striations on land engraved areas (LEA; the area between two grooves) are assumed to be unique to the individual barrel. This allows forensic examiners to determine whether two bullets originate from the same source by seeing if these LEAs match [@afte]. However, this process is highly subjective, and relies heavily on an examiner's expertise, leading to criticism by the @nas2009 and the @pcast. In response, algorithms for assessing  the similarity of firearms evidence were developed [@carriquiryMachineLearningForensic2019; @chenFiredBulletSignature2019; @chuAutomaticIdentificationBullet2013; @juOpenSourceImplementationCMPS2022; @vorburgerApplicationsCrosscorrelationFunctions2011; @vorburgerTopographyMeasurementsApplications2015] to provide objective measures with the goal of augmenting an examiner's testimony.

Algorithmic comparison methods have demonstrated considerable potential to quantify the similarity between pairwise pieces of evidence. However, current approaches have created a gap between the statistical metrics and the practical understanding of these metrics by practitioners creating the need for a more effective method to assist forensic practitioners in assessing and understanding the algorithm's performance. 

Here, we are proposing the **Forensic Bullet Comparison Visualizer** (FBCV), as an interactive interface designed to visualize the  statistical metrics embedded in the context of the data [@wickhamVisualizingStatisticalModels2015] in a manner that is intuitive and accessible to forensic examiners. FBCV leverages linked interactive graphics to allow forensic examiners to engage with the complex algorithmic data at each stage of the process, thereby filling the gap between statistical analysis and practical forensic application.

<!-- Interactive graphics section -->
Interactive graphics allow the user to dynamically explore and manipulate graphical representations of data by means of direct manipulation of a chart's graphical elements -- usually through clicking, dragging, or rescaling [@eickHighInteractionGraphics1995; @swayneIntroductionSpecialIssue1999; @urbanekIPlotsEXtremeNextgeneration2011]. A typical strategy for visualizing complex data is to use high-dimensional plots, incorporating multiple variables and levels of aggregation. However, these plots often pose challenges in interpretability, especially among those unfamiliar with the data [@willsLinkedDataViews2008]. To improve readability, a cogent method is to use linked data views of simpler charts. Linked data views communicate with each other and the underlying data and respond to user interactions by updating their graphical elements accordingly. This linkage system allows for the display of  interpretable graphs while preserving all relevant information [@willsLinkedDataViews2008]. The FBCV leverages linked views to present data at multiple levels of the bullet analysis process. 
...
We apply the following steps to each of the 3,120 scans [steps 1-4] and each pair of scans [step 5]. The results from these comparisons are then rendered in our visualization framework [[@sec-visuals]]. 
...
#| fig-cap: "Aligned Signals from lands of two separate bullets (from barrel A)."
#| fig-align: "center"
#| fig-height: 3.25
#| fig-width: 7
#| out-width: '90%'
  ...
  bullets <- readRDS(here::here("models/sampleBullets.rds"))
  
  #Aligning the signatures and extracting features
  bullets$bulletland <- paste0(bullets$bullet,"-", bullets$land)
  lands <- bullets$bulletland
  comparedBullets <- data.frame(
    expand.grid(land1 = lands[1], land2 = lands[2]), stringsAsFactors = FALSE)
  
  
  # Extract aligned signatures
  comparedBullets <- comparedBullets %>% mutate(
  ...
  # Plot the alignment
  d <- comparedBullets$aligned[[1]]
  
  d$lands %>%
    tidyr::gather(sigs, value, sig1, sig2) %>%
    mutate(
      Land = ifelse(sigs=="sig1", "B11-L6", "B12-L6")
    ) %>%
    ggplot(aes(x = x/1000, y = value, colour = Land)) + 
    geom_line() +
    theme_bw() +
    scale_color_brewer(palette = "Dark2") +
    labs(color = "Signals", 
         y = "(Relative) Surface height in microns [µm]",
         x = "(Relative) Horizontal extent in millimeters [mm]") +
    ylim(c(-5,5))

```

1.  A 3d LEA scan (left in @fig-extracted-sigs) is inspected for its suitability for comparisons, scans of low quality or of damaged lands (due to tank 'rash', pitting, or cracks) are declared 'unsuitable for comparison' and removed from the analysis.
...
4. The signal for a land (shown on the right of @fig-extracted-sigs) is created by removing the bullet curvature from the profile using a non-parametric smooth [@clevelandRobustLocallyWeighted1979].
...
Common examples of such algorithms include random forests [@hofmannBulletxtrctrAutomaticMatching2022] and congruent matching profile segments [@chenFiredBulletSignature2019; @juOpenSourceImplementationCMPS2022]. A large number of these algorithms are based on the maximized cross-correlation function between pairs of signals. This is the metric which we will use in this paper. However, this is not an actual restriction, any other similarity metric would work similarly well, with its usability only restricted by the metric's  ability to distinguish between marks from the same source and marks from different sources. 

Assume that $X = \left\{X_t\right\}_{1 \le t \le N_X}$ and $Y = \left\{X_s\right\}_{1 \le s \le N_Y}$ are the observed surface measurements (signals) of two land engraved areas (with $N_X, N_Y$ the number of the respective observations). The (theoretical) correlation between $X$ and $Y$ is defined as the ratio of their covariance scaled by their respective variances: 
<!-- -->
$$
\text{corr} (X, Y) = \frac{\text{Cov(X, Y)}}{\sqrt{\text{Var}(X) \text{Var}(Y)}}
$$ 
Let $Y^{(k)}$ define the $k$th lag of $Y$ with $Y^{(k)}_s = Y_{k+s}$ with $k \in [-M, M]$ and $0 \le M < N_Y$.  With that we define the ($M$-bound) maximized cross-correlation function $CCF_{\text{max}} (X, Y)$ as
<!-- -->
$$
CCF_{\text{max}} (X, Y) = \text{arg} \text{max}_{k \in [-M, M]} \text{corr}(X, Y^{(k)}).
$$
The choice of $M$ depends on the minimal number of values $N_Y-M$ used as a basis for the empirical estimation of the cross-correlation between the two signals.
Here, we use $M=500$  for the alignment of signals. This corresponds to a horizontal shift of $\pm 500$ values (equal to $\pm 500 \times 0.645 \mu\text{m} = .323$ mm) corresponding to about a quarter of a scan's width.


<!-- Visualization Development -->

When assessing the similarity of one bullet to another, a common approach is to assemble all scores from comparing pairs of lands in form of a square matrix and visualize it in form of a tile plot, see @fig-land-matrix. The fill color encodes the score (here, the empirical CCF$_\text{max}$) between a pair of LEAs. Higher values indicate higher similarity, shown in shades of orange. Tiles filled with grey values indicate less similarity.  The two bullets shown in the example are known to have been fired through the same barrel. In this case, we expect six pairs of lands with high similarities (in-phase, framed and marked by 'x'), while all other pairs (out-of phase) should result in low scores. This is exactly the pattern that can be seen in @fig-land-matrix. 
...
#| fig-cap: "Tile plot of all pairwise comparisons of LEA signals from two different bullets fired through the same barrel."
#| fig-align: "center"
#| fig-width: 3.15
#| fig-height: 2
#| out-width: "50%"
...
houstontemp %>%
  ...
  geom_tile(data = houstontemp |> dplyr::filter(samesource), fill=NA, colour = "darkorange", linewidth=1) +
  geom_text(aes(label="x"), data = houstontemp |> dplyr::filter(samesource), colour = "black",
            nudge_y=0.08) +
...
The empirical maximized CCF is a measure for the similarity for a pair of lands, for a bullet-level evaluation, we compute the average of in-phase scores [@songProposedBulletSignature2000a], as these reflect bullet similarity while also being highly indicative of model performance. This is given as

$$
\mathrm{\overline{CCF}_{max}}(B_1, B_2) = \underbrace{\left[  \frac{1}{n} \sum_{(i,j) \in \mathcal{P}} c_{ij}\right]}_{\text{in-phase average}},
$$

where $c_{ij}$ is the score between land $i$ on bullet 1 and land $j$ on bullet 2, with $1 \le i, j \le n=6$, and the phase $\mathcal{P}$ denotes the pairs of lands that capture the best alignment between bullets $B_1$ and $B_2$.
...
As seen in the previous section, similar there are similarity scores at the bullet-to-bullet level, there are scores at the land-to-land level, and there are important diagnostics for individual pairs of lands. For any given comparison, we have pertinent information at each of these levels (see @fig-tool-pipeline).
...
Clicking on the *lineplot* tab brings up the same scatterplot, with the key distinction that points representing the same second bullet in the comparison are connected by a line. This visualization helps to emphasize the relationships and trends between the points that share the same bullet. In @fig-scatterplot all comparisons with bullet 46 are highlighted. We see, that this bullet is generally involved in low scores, i.e.\ is not as similar to any of the bullets from this barrel.
<!-- -->
\hfill\newline
**Variograms** are used to represent values as a function of the  distance. The main claim of firearm examination is that the rifling marks left on a bullet are unique to the barrel and 'stable' over time. Statistically, this translates to the claim that there are no changes to the barrel by consecutive firings or, more realistically,  that changes to the rifling are small. In the context of bullet comparisons, the variogram illustrates how the similarity of bullets is affected by the number of bullets fired between them, shown in @fig-variogramA. The $x$-axis represents the numerical distance between shots (11 vs. 12 corresponds to a distance of 1, while 11 vs. 50 is a distance of 39). The $y$-axis represents the algorithmic score between the bullets. The blue line shows a loess fit to capture the main trend. Clicking on any point within the variogram leads to the same interactive pipeline as the scatterplots and other visualizations. In the variogram for barrel A, we see relatively large variability in the scores of bullets fired from the same barrel. The main trend has a slight negative slope indicating that the similarity between bullets decreases, on average, as the distance between fired shots increases. 
...
We used a combination of HTML and R for the implementation. Specifically we used ggplot2 [@wickhamGgplot2ElegantGraphics2016] as the basis for all visualizations, plotly [@sievertInteractiveWebbasedData2020] for variograms and scatterplots, and the HTML area tag for connecting cells in tile plots to linked plots. We used customized Javascript to enable additional linking.

All four types of visualizations are integrated into a single HTML website using htmlwidgets [@vaidyanathanHtmlwidgetsHTMLWidgets2023], providing a comprehensive view of the data and offering accessible insights into scores. 

## Use Cases

### The Case of Barrel D

The provided visualizations have shown scores for barrel A, but not all barrels displayed such straight-forward results. One such case was the scores for barrel D. After running the pairwise comparisons and analyzing the visualization framework, it became apparent that there was something strange going on with the scores for bullets 35-40. These bullets had relatively high scores compared to each other but scored fairly low when compared to any of the other bullets fired from barrel D, as shown in @fig-matrixD. In the next section, we lay out our use of different strategies to get to the bottom of this data discrepancy by comparing two control bullets (33-34) and the affected bullets (35-40) to different alternative sources. The bullets primarily used in this analysis are highlighted in @fig-matrixD.
...
#Reading in the background data and rescanned lands and grooves
  ...
  houstonDComparisons <- houstonDComparisons %>% mutate(analysisType = "Grooves of D")
...
houstonChanges <- readr::read_csv(here::here("data/houston-comparisons-D2.csv"))
  ...
  houstonDRescans <- houstonDRescans %>% mutate(analysisType = "Rescans of D")
...
#Reading in the background data and comparisons between barrels C and D
  ...
  houstonC <- houstonC %>% mutate(analysisType = "Barrel C")
...
#| fig-cap: "Overview of the adjustments made throughout the data cleaning process and their impact on the tile plots"
#| fig-height: 5
#| fig-width: 11
...
One potential reason for the suboptimal performance of bullets 35-40 may stem from the groove-engraved areas (GEAs) being scanned rather than the land-engraved areas. Note that groove engraved areas are usually not used for examinations because grooves are susceptible to sublass characteristics, i.e.\ they preserve marks from the broaching tool used to create the  grooves for the rifling. This incorporates marks specific to the tool on the surface of the barrel. Because the same broaching tool is used for multiple barrels, marks on grooves are not specific to a single barrel.
...
<!-- -->
Another measure in our investigation was to rescan and process the 3D topographical imaging on the bullets in question. The same control and test groups were used as the groove comparisons. Bullets 33 - 36 were rescanned, with 33 and 34 as the control. Replacing those comparisons in the interactive framework showed a drastic difference in results. In the matrix, the max phase correlation scores of the rescanned bullets aligned closely with those the other bullets fired from barrel D, shown in the middle plot of @fig-matrixDAdjustments. Furthermore, no significant change was found in the scores of the control group. When utilizing other parts of the FBCV, such as the 6x6 matrix and looking at the raw scans, these comparative results were reinforced. The alignment of signals for the rescans showed significant improvement compared to the previous alignment among barrel D.
...
<!-- -->
To answer whether the data was mislabeled, we compared the original scans of bullet D to data obtained from all of the 12 other barrels in the Houston dataset. We selected one bullet from each barrel that performed well when compared to the other bullets from that barrel (12 bullets in total). Those bullets were then compared to both each other and bullet 39 from barrel D, one of the originally poor-performing bullets.

We found that 11 of the 12 bullets showed poor performance when compared to the selected bullet from barrel D. The exception was the bullet selected from barrel C. When comparing that bullet to bullet 39 of barrel D, the algorithmic results showed that the two bullets were likely fired through the same barrel. @fig-CD-Comparison shows the 6x6 matrix from our framework tool when comparing the selected bullet from barrel C to bullet 39 of barrel D. The alignment between these two bullets is extremely strong, even stronger than many other bullet comparisons where both bullets originated from barrel C.
...
It is also important to note that the selected bullets did not perform well when compared with each other (i.e., results from the chosen bullet from barrel A did not match the bullet from barrel B). This implies the similarity between the selected bullet from barrel C and the poor-performing bullet from barrel D cannot be attributed to well-performing bullets being accurate among comparisons with other weapons. Thus, we have strong evidence that the bullets were mislabeled, with the poorly performing bullets originating from barrel C.

This notion is strengthened by our visualization tool. When the comparisons for bullets 35-40 are substituted with those initially labeled as bullets 35-40 fired by barrel D, there is no significant deviation in performance according to the matrix shown in the bottom plot of @fig-matrixDAdjustments. Overall, these scans perform very well compared to other bullets shot by barrel C. Thus, we can conclude that bullets 35-40 were mislabeled and shot by barrel C, not D.


### Bullet Distance Analysis

Firing shots from the same weapon while tracking the number of shots allows an investigation of the stability of the marks engraved on bullets over time.  Using variograms, we provided a visual representation of this performance.  @fig-variograms illustrates variograms for all barrels in the Houston dataset. Note that poor-performing bullets from barrel D were removed in this analysis, as well as two bullets from barrel J that displayed similar performance issues. <!--Future analysis will investigate the underlying cause of these discrepancies in bullet J.-->


```{r}
#| label: fig-variograms
#| fig-cap: "Variograms across all barrels"
#| fig-align: "center"
#| fig-height: 7
#| fig-width: 8
...
$\theta_0$ gives a baseline score for each individual barrel, while
$\theta_1$ represent the coefficient associated with the change in mean $CCF_{max}$ score of bullets at a given $d$. 
...
It is apparent that there are differences in baseline model performance between different barrels. $\theta_0$ ranges from `r round(min(model_table$intercept),2)` for barrel B to `r round(max(model_table$intercept), 2)` for barrel J, with a mean of `r sprintf("%.2f", mean(model_table$intercept))`.

We expect that the similarity between shots will decline as the distance between shots  increases. We see this performance in most barrels, however, $\theta_1$ is quite small for all barrels, falling between `r min(model_table$slope)` and `r sprintf("%.5f", max(model_table$slope))`. 


Combined with the visual analysis of the variogram, we see that bullet distance has only a small effect on similarity between bullets relative to other causes of variability. This variability suggests that additional factors beyond bullet distance have a substantial effect on model performance and accuracy. This is evident when $d < 30$, whereas beyond $d > 30$, there is insufficient data to make definitive claims. 

<!--
While the distance between shots may not be a significant factor in predictive performance, we recognize there is significant amount of variability among in-phase scores between shots. Potential factors may include, but are not limited to, inconsistencies in bullet scan accuracy, slight differences in ammunition characteristics, and differences in the way in which a barrel was discharged, may also play a key role in observed variability. These factors prompt further investigation to determine their role in achieving consistent model predictions and reducing variability among bullets.
-->
...
The FBCV also includes variograms, which allow for insightful analysis on how bullet distance affects model accuracy. Using these visualization tools, we examined the impact on 13 different barrels. By fitting a line to the mean in-phase score at each distance among variograms, we observed that model accuracy does not significantly degrade with increasing bullet distance. However, the variability of bullet scores is high, indicating other factors have a substantial effect on prediction performance outside of how many shots were fired between bullets. The plots included in the FBCV offer a promising tool for future analysis on additional datasets, which could contribute to expanding the literature in this area.

### Limitations
This study faces a number of limitations, chiefly regarding file storage. Because of the large number of comparisons, a considerable amount of files were rendered for each stage in the FBCV's process. Each barrel contains 1600 bullet-to-bullet comparisons. Then, for each bullet-to-bullet comparison, there are 36 sub-comparisons made at the land level, corresponding to the six lands of each bullet. Therefore, we must process $1600 \times 36 = 57,600$ png images at the land level.
...
Despite these adjustments, the substantial number of files presents a significant storage challenge, and Github was unable to accommodate more than the number of files associated with one barrel. Consequently, the figures presented in this analysis primarily focus on barrel A. Furthermore, the need to render such a large number of files complicates the process of making rapid updates to the scans, as any modification typically requires re-rendering a large number of figures. Still, as demonstrated in previous examples, the framework does allow for updates as changes are introduced.

### Future Work
Future research in this area should focus on exploring the underlying causes of variability between shots and identifying the factors contributing to this variability. This is crucial to improving model performance and establishing confidence within the forensic community regarding the adoption of such algorithmic models. Additionally, exploring the impact of firearm storage on bullet performance presents a valuable direction for future research.

## Conclusion

This paper presented an interactive framework for analyzing algorithmic comparisons of whether two bullets were fired from the same barrel. The framework includes various visualizations that allow the user to assess algorithmic performance at a broader scope while also diagnosing issues at every level of the comparative analysis process. The FBCV was used to analyze algorithmic performance on the Houston dataset. It successfully identified a problematic error in the comparison process, and investigative steps were taken to discern the cause of the error, which is attributed to mislabeling. In the future, this visualization framework can provide summary overviews of algorithmic performance and diagnose problems in the data processing of forensic bullet analysis. By offering an interface that is intuitive and accessible, it presents an option that can support forensic examiners and lead to more accurate forensic analysis.
