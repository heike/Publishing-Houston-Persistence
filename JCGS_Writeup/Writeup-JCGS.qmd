...
matrix <- barrelD %>%
  ...
  geom_tile() +
  # Add red rectangle from x = 33 to 40
  geom_rect(
...
#Preprocessinng
 ...
 matrix <- houstonP2 %>%
  ...
  scale_colour_manual(values = c("grey80", "darkorange")) +
  scale_x_discrete(
    limits = as.character(33:40),
    breaks = as.character(c(33, 35, 37, 40)), 
    name = "Bullet 1"
  ) +
  scale_y_discrete(
...
matrixDGrooves + plot_spacer()  + matrixDreplacement + plot_spacer()  + matrixCD + plot_layout(guides = 'collect', ncol = 5, nrow = 1, widths = c(13, -5, 13, -5, 13))
