  ...
  - name: Nathan Rethwisch
    ...
    affiliations:
        - id: nreth
          ...
          address: 2438 Osborn Dr, Ames IA, 50010
    attributes:
          corresponding: true
  - name: Heike Hofmann
    ...
    affiliations:
        ...
        - id: hofmann
          ...
          department: Department of Statistics
          address: 3310 Holdrege St, Lincoln, NE, 68583
    attributes:
          corresponding: false
    # note: |
abstract: |
...
Identifying the firearm used in a crime is a critical component of forensic investigations and plays a pivotal role in criminal investigations. Ensuring evidence is appropriately identified is crucial in upholding the integrity of the criminal justice system.
...
#| fig-align: "center"
knitr::include_graphics("../figure/Aligned signatures (1).png")
...
```{r fig-land-matrix, fig.cap = "Tile plot of all pairwise comparisons of LEA signals from two different bullets.", fig.width = 3, fig.height = 1.85, out.width='27%'}
...
<!--The in-phase score is represented by the mean of the diagonal with the highest average score, while the out-of-phase score is represented by all 30 observations not included in the in-phase diagonal.-->
...
<!--
We developed an interactive visualization tool that offers a graphical representation of this maximum phase score, enabling large-scale analysis and a deeper understanding of individual metrics. 
This pipeline is shown in @fig-tool-pipeline.
-->
<!-- ## Max Phase Correlation Score-->
<!--
Of particular interest is drawing inferences on algorithmic results among many pairwise comparisons related to each firearm. Current visualization techniques analyze how six LEAs from one bullet compare against six LEAs from another. While this is an effective way to visualize the validity of algorithmic comparisons between two bullets, this method is less than adequate when there a large number of bullets are fired, such as the $^{40}P_2 = 1560$ comparisons seen per bullet in the Houston dataset. 
This statistic allows for a comprehensive overview of how algorithmic comparisons are performing on the data as a whole, providing opportunities for analysis of accuracy over the entirety of the Houston dataset, as well as providing key information on performance deviation over time.
-->
The forensic bullet comparison visualizer (FBCV) is created in HTML using a combination of Javascript and R code. This allows us to leverage the everyday familiarity of links for implementing connections across levels of information. An implementation of the FBCV showing all comparisons involving bullets from barrel A can be found at https://tinyurl.com/y53n3mkm. 
...
```
<!-- -->
The main interface of the FBCV consists of a set of tabs with choices for the visualization of the set of bullet scores, see @fig-framework-interface,
<!--Each tab shows an interactive visualization that focuses on a different aspect of the data--> discussed in more detail next.
\hfill\newline
<!-- ## Tile plots -->
...
<!--An example of this new clustering is shown in @fig-ClusteredMatrix. The ordering of the bullets in the matrix follows the ordering of the clustered dendrogram, displayed in @fig-dendrogram. -->

```{r fig-ClusteredMatrix, fig.cap = "Re-ordered tile plot for barrel A scores. ", fig.width = 4.1, fig.height = 2.75, out.width='40%', fig.align='center'}
...
```{r dendrogram, include=FALSE, fig.cap = "The dendrogram for hierarchical clustering"}
...
#Export this to png and then crop off the extra whitespace
```

![The dendrogram for hierarchical clustering](../figure/dendrogram.png){#fig-dendrogram}


<!-- The clustered matrix retains the same interactive pipeline as the unclustered matrix. By clicking on a specific box within the clustered matrix, the user is directed to the corresponding 6x6 matrix, which then links to additional information regarding the land-specific comparisons. -->
...
```{r fig-variogramA, fig.cap = "The variogram included in the visualization framework"}
